...
where
  ...
  circ.getNumberOfQubits() > circ.getNumberOfClassicalBits() and
  // the circuit is not a subcircuit
  not circ.isSubCircuit() and
...
select circ,
  "Circuit '" + circ.getName() + "' has more qubits (" + circ.getNumberOfQubits() +
    ") than classical  bits (" + circ.getNumberOfClassicalBits() + ")"
