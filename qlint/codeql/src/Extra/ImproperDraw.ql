...
where
  ...
  exists(Name n |
    printCall = e.getValue() and n = printCall.getFunc() and n.getId() = "print"
  ) and
...
select drawCall, "The draw function does not return a string, but it has to be used as is: e.g. replace print(qc.draw(output='mpl')) with qc.draw(output='mlp')."
