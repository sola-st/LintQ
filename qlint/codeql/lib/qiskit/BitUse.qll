...
class QubitUseViaAttribute extends QubitUse {
  ...
  QubitUseViaAttribute() {
    exists(
      ...
      DataFlow::LocalSourceNode locSource, DataFlow::CallCfgNode call
    |
      // detect qc.h(0)
    ...
    |
      exists(int i | i = gs.getAnArgumentIndexOfQubit() |
        call.(API::CallNode).getParameter(i)
          .getAValueReachingSink() = locSource
      )
      or
      exists(string kyw | kyw = gs.getAnArgumentNameOfQubit() |
        call.(API::CallNode).getKeywordParameter(kyw)
          .getAValueReachingSink() = locSource
      )
    )
  }
  ...
  override int getAnIndexIfAny() {
    ...
    // > 7
    if exists(IntegerLiteral lit, DataFlow::LocalSourceNode litSource |
    ...
    then
      ...
      then
        ...
        then exists(RegisterV2 reg, int i |
  ...
  override RegisterV2 getARegister() {
    ...
    then exists(RegisterV2 reg |
      ...
      if exists(RegisterV2 reg | reg.flowsTo(this))
      then exists(RegisterV2 reg | reg.flowsTo(this) |
          result = reg
        )
  ...
  override QuantumCircuit getACircuit() {
    exists(
      QuantumCircuit circ, GateSpecification gs, DataFlow::CallCfgNode call
    |
      // detect qc.h(0)
      call = circ.getAnAttributeRead(gs).getACall() and
      (
...
abstract class GateSpecification extends string {
  ...
  int getAnArgumentIndexOfClbit() {
    exists(int i |
      ...
      if (this.getNumberOfQubits() > 0) then
        result = i + this.getNumberOfQubits()
      else
        result = i
    )
  }
  ...
  int getAnArgumentIndexOfQubit() {
    exists(int i |
      this.getNumberOfQubits() > 0 and i in [0 .. this.getNumberOfQubits() - 1]
    |
      // shift the index if there are parameters
      if (this.getNumberOfParams() > 0) then
        result = i + this.getNumberOfParams()
      else
        result = i
      )
  }
  ...
  int getAnArgumentIndexOfParam() {
    exists(int i |
      ...
      i in [0 .. this.getNumberOfParams() - 1] | result = i)
  }
  ...
  int getNumberOfBits() {
    result = this.getNumberOfQubits() + this.getNumberOfClbits()
  }

  /** Number of parameters used in the gate.*/
  int getNumberOfParams() {
    result = count(string s | s = this.getAnArgumentNameOfParam())
  }

  /** Number of classical bits used in the gate.*/
  int getNumberOfClbits() {
    result = count(string s | s = this.getAnArgumentNameOfClbit())
  }

  /** Number of qubits used in the gate.*/
  int getNumberOfQubits() {
    result = count(string s | s = this.getAnArgumentNameOfQubit())
  }
...
class GateSpecificationAttributeName extends string {
  GateSpecificationAttributeName() {
    this in [
      ...
      "x", "y", "z", "h", "s", "sdg", "t", "tdg", "rx", "ry", "rz", "rv", "u1", "u2", "u3",
      "id", "i", "sx",
...
class GateSpecificationSingleQubitNoParam extends GateSpecification {
  GateSpecificationSingleQubitNoParam() {
    this in [
      "h", "x", "y", "z", "s", "sdg", "t", "tdg", "sx", "i", "id",
      "HGate", "XGate", "YGate", "ZGate", "SGate", "SdgGate", "TGate", "TdgGate", "SXGate", "IGate"
    ]
  }
...
class GateSpecificationCXGate extends GateSpecification {
  ...
  override string getAnArgumentNameOfQubit() {
    result in ["control_qubit","target_qubit"]
  }
}
